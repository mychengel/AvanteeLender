...
public class AuthenticationRepository {
    ...
    public MutableLiveData<JSONObject> reregistType1(final String uid, final String token, final String ctype, final Context context) {
        final MutableLiveData<JSONObject> result = new MutableLiveData<>();
        requestQueue = Volley.newRequestQueue(context, new HurlStack());
    ...
    public MutableLiveData<JSONObject> reregistType23(final String uid, final String token, final String ctype, final Context context) {
        final MutableLiveData<JSONObject> result = new MutableLiveData<>();
        requestQueue = Volley.newRequestQueue(context, new HurlStack());
        final VolleyMultipartRequest jor = new VolleyMultipartRequest(Request.Method.POST, url+"merchand/privy/reregister",
                new Response.Listener<NetworkResponse>() {
        ...
        )
        {
            ...
            @Override
            protected Map<String, DataPart[]> getByteDataArray() {
                Map<String, DataPart[]> params = null;
                if(ctype.equalsIgnoreCase("perorangan")){
                    params = GlobalVariables.perReregDataFileArray;
                }else{
                    params = GlobalVariables.insReregDataFileArray;
                }
                return params;
            }
